...
def prepare_train_data(training_dir):
    ...
    #Now, we can convert our Pandas DataFrames into MXNet NDArrays,
    #use those to create a member of the SparseMatrixDataset class, 
    #and add that to an MXNet Data Iterator. This process is the same for both test and control.
...
def model_fn(model_dir):
    ...
    #We will reconstruct the skeleton of our Neural Net using the MFblock class we created earlier and load the 
    #parameters from the model.json we have in the model directory using the load_params function.
